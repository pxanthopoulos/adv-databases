...
spark = SparkSession.builder.appName("part5-df-bcast").getOrCreate()
...
df_descent_zip = (
    ...
    .join(revgeocoding_df.hint("broadcast"), (revgeocoding_df["LAT"] == dataset_df["LAT"]) & (revgeocoding_df["LON"] == dataset_df["LON"]), "inner")
    .withColumn("Zip code", F.col("ZIPcode").substr(1, 5).cast("int"))
    .select(["Vict Descent", "Zip code"])
)

print("CRIME - ZIP JOIN: BROADCAST")
df_descent_zip.explain()

df_descent_income = (
    df_descent_zip
    .join(income2015_df.hint("broadcast"), income2015_df["Zip Code"] == df_descent_zip["Zip code"])
    .select(["Vict Descent", "Estimated Median Income"])
    .withColumn("Victim Descent", descript_udf(F.col("Vict Descent")))
)

print("CRIME - INCOME JOIN: BROADCAST")
